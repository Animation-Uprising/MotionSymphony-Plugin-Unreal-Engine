...
FMotionTimeSliderController::FMotionTimeSliderController(const FTimeSliderArgs& InArgs, /*TWeakPtr<FAnimModel> InWeakModel,*/
...
FReply FMotionTimeSliderController::OnMouseMove(SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	else if (bHandleLeftMouseButton)
	{
		...
		if (MouseDragType == DRAG_NONE)
		{
			if (DistanceDragged > FSlateApplication::Get().GetDragTriggerDistance())
			{
				UAnimMontage* AnimMontage = nullptr;// Cast<UAnimMontage>(WeakModel.Pin()->GetAnimSequenceBase());
