...
struct FMotionPaintSectionAreaViewArgs
{
	FMotionPaintSectionAreaViewArgs()
...
class FMotionTimeSliderController : public ITimeSliderController
{
...
private:
	...
	TWeakPtr<SMotionTimeline> WeakTimeline;
...
public:
	FMotionTimeSliderController(const FTimeSliderArgs& InArgs, TWeakPtr<FMotionModel> InWeakModel, 
...
private:
	...
	void DrawTicks(FSlateWindowElementList& OutDrawElements, const TRange<double>& ViewRange, 
		const FScrubRangeToScreen& RangeToScreen, FDrawTickArgs& InArgs) const;

	int32 DrawSelectionRange(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, 
		int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args) const;
	int32 DrawPlaybackRange(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, 
		int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args) const;
