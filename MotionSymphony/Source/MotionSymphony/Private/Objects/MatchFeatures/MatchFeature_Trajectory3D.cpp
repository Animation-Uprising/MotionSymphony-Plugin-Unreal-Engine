...
bool UMatchFeature_Trajectory3D::IsSetupValid() const
{
	bool bIsValid = true;
...
int32 UMatchFeature_Trajectory3D::Size() const
{
	return TrajectoryTiming.Num() * 5;
}
...
void UMatchFeature_Trajectory3D::SourceInputData(TArray<float>& OutFeatureArray, const int32 FeatureOffset, AActor* InActor)
{
	if(!InActor)
	{
	...
	if(UTrajectoryGenerator_Base* TrajectoryGenerator = InActor->GetComponentByClass<UTrajectoryGenerator_Base>())
	{
		...
		for(int32 i = 0; i < Iterations; ++i)
		{
			...
			const int32 PointOffset = FeatureOffset + i;
			if(PointOffset + 4 >= OutFeatureArray.Num())
			{
				UE_LOG(LogTemp, Error, TEXT("UMatchFeature_Trajectory3D: SourceInputData(...) - Feature does not fit in FeatureArray"));
				return;
			}
			
			OutFeatureArray[PointOffset] = TrajectoryPoint.Position.X;
			OutFeatureArray[PointOffset + 1] = TrajectoryPoint.Position.Y;
			OutFeatureArray[PointOffset + 2] = TrajectoryPoint.Position.Z;
			OutFeatureArray[PointOffset + 3] = RotationVector.X;
			OutFeatureArray[PointOffset + 4] = RotationVector.Y;
		}
	}
}

void UMatchFeature_Trajectory3D::ApplyInputBlending(TArray<float>& DesiredInputArray,
                                                    const TArray<float>& CurrentPoseArray, const int32 FeatureOffset, const float Weight)
{
	if(DesiredInputArray.Num() < TrajectoryTiming.Num() * 5
	...
	{
		const int32 StartIndex = FeatureOffset + i * 5;
		...
		if(Time > 0.0f)
		{
			...
			DesiredInputArray[StartIndex+4] = FMath::Lerp(CurrentPoseArray[StartIndex+4], DesiredInputArray[StartIndex+4], Progress);
		}
	}
}

bool UMatchFeature_Trajectory3D::NextPoseToleranceTest(const TArray<float>& DesiredInputArray,
	...
	const float PositionTolerance, const float RotationTolerance)
{
	...
	for(int32 i = 0; i < Iterations; ++i)
	{
		...
		SqrDistance += FMath::Abs(DesiredInputArray[PointIndex+4] - PoseMatrix[MatrixIndex+4]);
...
float UMatchFeature_Trajectory3D::GetDefaultWeight(int32 AtomId) const
{
...
void UMatchFeature_Trajectory3D::CalculateDistanceSqrToMeanArrayForStandardDeviations(
	...
	const int32 FeatureOffset, const int32 PoseStartIndex) const
{
	for(int32 i = 0; i < TrajectoryTiming.Num(); ++i)
	{
		const int32 MeansStartIndex = FeatureOffset + i*5;
		...
		FVector2d PointFacing
		{
			...
			InPoseArray[PointStartIndex + 4]
		};

		FVector2d MeanPointFacing
		{
			InMeanArray[MeansStartIndex + 3],
			InMeanArray[MeansStartIndex + 4]
		};
		
		const float DistanceToMean_Location = FVector::DistSquared(PointLocation, MeanPointLocation);
		...
		OutDistToMeanSqrArray[MeansStartIndex + 2] += DistanceToMean_Location;
		OutDistToMeanSqrArray[MeansStartIndex + 3] += DistanceToMean_Facing;
		OutDistToMeanSqrArray[MeansStartIndex + 4] += DistanceToMean_Facing;
	}
}
	...
	const UWorld* World, FPrimitiveDrawInterface* DrawInterface)
{
	...
	FVector LastPointPos = PreviewTransform.TransformPosition(FVector(PoseArray[StartIndex],
		PoseArray[StartIndex+1], PoseArray[StartIndex+3]));
	...
	for(int32 i = 0; i < TrajectoryTiming.Num(); ++i)
	{
		...
		FVector RawPointPos(PoseArray[PointIndex], PoseArray[PointIndex + 1], PoseArray[PointIndex + 2]);
		...
		FVector RawArrowVector(PoseArray[PointIndex + 3], PoseArray[PointIndex + 4], 0.0f);
...
void UMatchFeature_Trajectory3D::DrawDebugDesiredRuntime(FAnimInstanceProxy* AnimInstanceProxy,
	FMotionMatchingInputData& InputData, const int32 FeatureOffset, UMotionMatchConfig* MMConfig)
{
	...
	for(int32 i = 0; i < TrajectoryTiming.Num(); ++i)
	{
		const int32 StartIndex = FeatureOffset + i * 5 - 1;
		
		if(StartIndex + 4 >= InputArray.Num())
		{
			break;
		}
		...
		FVector DrawTo = PointPosition + MeshTransform.TransformVector(FacingOffset *
			FVector(InputArray[StartIndex+3], InputArray[StartIndex+4], 0.0f) * 30.0f);
                                                         ...
                                                         TObjectPtr<const UMotionDataAsset> MotionData, const TArray<float>& CurrentPoseArray, const int32 FeatureOffset)
{
	...
	for(int32 i = 0; i < TrajectoryTiming.Num(); ++i)
	{
		...
		FVector DrawTo = PointPosition + MeshTransform.TransformVector(FacingOffset *
			FVector(CurrentPoseArray[StartIndex+3], CurrentPoseArray[StartIndex+4], 0.0f) * 30.0f);
